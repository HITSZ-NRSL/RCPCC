...
int count_file_bytes(const std::vector<std::string> &filenames)
{
  int size = 0; // 文件大于2G会溢出
  for (auto filename : filenames)
  {
    std::ifstream in_stream(filename, std::ifstream::binary | std::ifstream::ate);
    std::streamsize in_size = in_stream.tellg();
    std::cout << filename << " size: " << in_size << "bytes" << std::endl;
    size += static_cast<int>(in_size);
  }
  return size;
}
